...
final class SearchService
{
    ...
    public function search(User $user, WorkspaceContext $context, string $query): array
    {
        ...
        return [
            'meetings' => Meeting::query()->where('user_id', $user->id)->where('workspace_context_id', $context->id)
                ->where('title', 'like', $like)->limit(10)->get(['id', 'title', 'starts_at'])->toArray(),
            'actions' => ActionItem::query()->where('user_id', $user->id)->where('workspace_context_id', $context->id)
                ->where('title', 'like', $like)->limit(10)->get(['id', 'title', 'status', 'due_date'])->toArray(),
            'documents' => DocumentReference::query()->where('user_id', $user->id)->where('workspace_context_id', $context->id)
                ->where('title', 'like', $like)->limit(10)->get(['id', 'title', 'provider', 'deep_link'])->toArray(),
            'people' => Person::query()->where('user_id', $user->id)->where('workspace_context_id', $context->id)
                ->where('name', 'like', $like)->limit(10)->get(['id', 'name'])->toArray(),
            'conversations' => Conversation::query()->where('user_id', $user->id)->where('workspace_context_id', $context->id)
                ->where('subject', 'like', $like)->limit(10)->get(['id', 'subject', 'channel'])->toArray(),
            'trips' => Trip::query()->where('user_id', $user->id)->where('workspace_context_id', $context->id)
